...
class TraversalTest extends TestCase {
    /**
     * Test a deep traversal of an Api Declaration
     *
     * @dataProvider provideApiDeclaration
     * 
    ...
    public function provideApiDeclarationDocument() {
        $data = array();
        
        foreach(glob(__DIR__.'/fixtures/apideclaration*.json') as $filename) {
            $data[] = array(
    ...
    public function provideApiDeclaration() {
        $documents = $this->provideApiDeclarationDocument();
        
        $data = array();
        
        foreach($documents as $document) {
            $data[] = array(
                new ApiDeclaration($document[0])
            );
        }
